...
async function fetchModules () {
  ...
  try {
    ...
    modulesError.value = e && e.data
      ? JSON.stringify(e.data)
      : (e && e.message ? e.message : 'Failed to load modules')
  } finally {
    modulesLoading.value = false
  }
}

async function startAttempt (moduleId) {
  ...
  try {
    ...
    const id = res.id || res.attempt_id || res.attemptId
    attemptId.value = String(id)
    activeModuleId.value = moduleId
    await loadNextQuestion()
  } catch (e) {
    globalError.value = e && e.data
      ? JSON.stringify(e.data)
      : (e && e.message ? e.message : 'Failed to start attempt')
  }
}
